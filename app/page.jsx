    ...
    <div className="space-y-6 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 ml-[52px]">Real-time flight deal intelligence — {monitoredRoutes.length} routes monitored</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { icon: Radar, label: 'Active Deals', value: enrichedDeals.length, color: 'text-white' },
          { icon: AlertTriangle, label: 'Error Fares', value: errorFares, color: 'text-accent-red', iconColor: 'text-accent-red' },
          { icon: TrendingDown, label: 'Price Drops', value: priceDrops, color: 'text-accent-green', iconColor: 'text-accent-green' },
          { icon: Zap, label: 'Avg Savings', value: `${avgSavings}%`, color: 'text-accent-amber', iconColor: 'text-accent-amber' },
        ].map(stat => (
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          {[
            ...
            { key: 'error_fare', label: 'Error Fares' },
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                filter === f.key ? 'bg-brand-500/20 text-brand-400' : 'text-gray-400 hover:text-gray-200 hover:bg-surface-hover'
              }`}
            >
              {f.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          ...
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                sortBy === s.key ? 'bg-surface-elevated text-white' : 'text-gray-500 hover:text-gray-300'
              }`}
            >
              {s.label}
            </button>
