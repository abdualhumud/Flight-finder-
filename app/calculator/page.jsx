    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="glass rounded-xl p-4 flex items-start gap-3 border border-accent-amber/20">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">After baggage, seat selection, and card surcharges, the "Final Landing Price" often favors direct booking.</p>
        </div>
      </div>
          ...
          <div key={route.route} className="glass rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-border-subtle flex items-center justify-between">
              ...
              <span className="flex items-center gap-1 text-xs text-accent-green font-medium">
                <Check className="w-3.5 h-3.5" /> Best: {winner} ({formatPrice(cheapest)})
              </span>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border-subtle">
                    {['Fee Breakdown', 'Direct Airline', 'Expedia', 'Skyscanner'].map(h => (
                      <th key={h} className={`${h === 'Fee Breakdown' ? 'text-left' : 'text-center'} px-4 py-2 text-[10px] text-gray-500 uppercase tracking-wider font-medium`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {['baseFare', 'baggage', 'seatSelection', 'cardSurcharge'].map(fee => {
                    const labels = { baseFare: 'Base Fare', baggage: 'Baggage (23kg)', seatSelection: 'Seat Selection', cardSurcharge: 'Card Surcharge' };
                    return (
                      <tr key={fee} className="border-b border-border-subtle">
                        <td className="px-4 py-2.5 text-xs text-gray-400">{labels[fee]}</td>
                        <td className="px-4 py-2.5 text-sm text-center text-gray-300">{formatPrice(route.direct[fee])}</td>
                        <td className="px-4 py-2.5 text-sm text-center text-gray-300">{formatPrice(route.expedia[fee])}</td>
                        <td className="px-4 py-2.5 text-sm text-center text-gray-300">{formatPrice(route.skyscanner[fee])}</td>
                      </tr>
                    );
                  })}
                  <tr className="bg-surface-hover">
                    <td className="px-4 py-3 text-xs font-semibold text-white flex items-center gap-1">
                      <ArrowDown className="w-3 h-3 text-brand-400" /> FINAL LANDING PRICE
                    </td>
                    {['direct', 'expedia', 'skyscanner'].map(src => (
                      <td key={src} className={cn('px-4 py-3 text-center font-bold', route[src].total === cheapest ? 'text-accent-green' : 'text-white')}>
                        ...
                        {route[src].total === cheapest && <Check className="w-3 h-3 inline ml-1 text-accent-green" />}
                      </td>
                    ))}
                  </tr>
                </tbody>
