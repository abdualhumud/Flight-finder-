...
import { upgradeTemplates } from '../data/mockData';
...
export default function UpgradeSuite() {
  ...
  const [formData, setFormData] = useState({
    ...
    name: '', loyaltyNumber: '', status: '', occasion: '',
  });
  ...
  function fillTemplate(template) {
    ...
    const replacements = {
      ...
      '[Flight Number]': formData.flightNumber || '[Flight Number]',
      ...
      '[Number]': formData.loyaltyNumber || '[Number]',
      '[Loyalty Program]': formData.airline ? `${formData.airline} Loyalty` : '[Loyalty Program]',
      '[Status Level]': formData.status || '[Status Level]',
      '[Home Airport]': formData.origin || '[Home Airport]',
      '[Competing Airline]': '[Competing Airline]',
    };
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1 ml-[52px]">Smart negotiation templates for upgrades and rebooking</p>
      </div>
      ...
      <div className="glass rounded-xl p-5">
        <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
          ...
          Your Flight Details (auto-fills templates)
        </h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { key: 'name', placeholder: 'Your Name' },
            { key: 'airline', placeholder: 'Airline' },
            { key: 'flightNumber', placeholder: 'Flight Number' },
            { key: 'date', placeholder: 'Travel Date', type: 'date' },
            { key: 'origin', placeholder: 'Origin (e.g. RUH)' },
            { key: 'destination', placeholder: 'Destination (e.g. LHR)' },
            { key: 'loyaltyNumber', placeholder: 'Loyalty Member #' },
            { key: 'status', placeholder: 'Status Level' },
          ].map(field => (
            <input
              key={field.key}
            ...
            <div key={tmpl.id} className="glass rounded-xl overflow-hidden">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Trigger: {tmpl.trigger}</p>
                </div>
                {isExpanded ? <ChevronUp className="w-4 h-4 text-gray-500" /> : <ChevronDown className="w-4 h-4 text-gray-500" />}
              </button>
              {isExpanded && (
                <div className="px-4 pb-4 border-t border-border-subtle pt-4">
                  <pre className="bg-surface rounded-lg p-4 text-xs text-gray-300 whitespace-pre-wrap font-mono leading-relaxed overflow-auto max-h-80">
                    {filledText}
                  </pre>
                  <button
                    onClick={() => copyToClipboard(filledText, tmpl.id)}
                  ...
                  >
                    {copied === tmpl.id ? <><Check className="w-3.5 h-3.5" /> Copied!</> : <><Copy className="w-3.5 h-3.5" /> Copy to Clipboard</>}
                  </button>
                </div>
              )}
            </div>
