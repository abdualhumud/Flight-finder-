    ...
    <div className={cn('relative', className)} ref={ref}>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-surface-hover border border-border-subtle rounded-lg ps-9 pe-3 py-3 md:py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-brand-500/50"
        />
        {isLoading && <Loader2 className="absolute end-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 animate-spin" />}
      </div>

      {isOpen && (query.length >= 2 || results.length > 0) && (
        <div className="absolute z-[60] w-full mt-1 bg-surface-elevated border border-border-subtle rounded-lg shadow-xl max-h-60 overflow-y-auto">
          ...
          {results.map((airport, i) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-3 md:py-2.5 text-start hover:bg-surface-hover transition-colors"
            >
              <Plane className="w-4 h-4 text-gray-500 flex-shrink-0" />
              <div className="min-w-0 flex-1">
                <div className="text-sm text-white truncate">{airport.name}</div>
                <div className="text-[10px] text-gray-500 truncate">{airport.subtitle}</div>
              </div>
              <span className="ms-auto text-xs font-mono text-brand-400 flex-shrink-0">{airport.iata}</span>
            </button>
          ))}
        </div>
      )}
